...
from networks.custom_dt_gini_with_entropy_metrics import \
    CustomDecisionTree
from networks.loss import CELoss, CustomConceptLoss, BCEWithLogitsLoss, \
    BCEWithLogitsLossWithEntropy
from utils import flatten_dict_to_list


class MIMICBlackBoxArchitecture:
...
class MIMICCBMwithSLRaslabelPredictorArchitecture:
    def __init__(self, config, device, hard_concepts=None, data_loader=None):
...
class MIMICBMwithDTaslabelPredictorArchitecture:
    def __init__(self, config, device, hard_concepts=None, data_loader=None):
...
class MIMICCBMArchitecture:
    def __init__(self, config, device, hard_concepts=None, data_loader=None):
        ...
        if 'entropy_layer' in config["model"]:
            ...
            self.label_predictor = LabelPredictorNN(concept_size=concept_size,
                                                    num_classes=config["dataset"]["num_classes"])
        else:
            self.label_predictor = LabelPredictorLR(concept_size=config["dataset"]["num_concepts"],
                                                  num_classes=config["dataset"]["num_classes"])
        self.model = MainNetwork(self.concept_predictor, self.label_predictor)
        ...
        self.criterion_concept = CustomConceptLoss(concept_names=concept_names)
        if 'entropy_layer' in config["model"]:
            self.criterion_label = BCEWithLogitsLossWithEntropy(lm=config["model"]["lm"],
...
class MIMICCYArchitecture:
    def __init__(self, config):

        self.model = LabelPredictorLR(concept_size=config["dataset"]["num_features"],
                                    num_classes=config["dataset"]["num_classes"])
...
class ConceptPredictor(nn.Module):
    def __init__(self, num_concepts):
        ...
        self.fc3 = nn.Linear(64, num_concepts)
...
class LabelPredictorNN(nn.Module):
    def __init__(self, concept_size, num_classes):
        ...
        self.fc1 = nn.Linear(concept_size, 128)
        self.fc2 = nn.Linear(128, 64)
